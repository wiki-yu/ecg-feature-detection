...
class Detectors:
    ...
    def ecg_detector(self, unfiltered_ecg):
        ...
        for i in range(len(low_pass)):
            ...
            if thi and i < thi_list[-1] + ms160:
                ...
                if low_pass[i] < -M and low_pass[i - 1] > -M:
                    print(
                        "## close to bottom, i:{}, -M:{}, low_pass[i]:{} ".format(
                            i, -M, low_pass[i]
                        )
                    )
                    # thf_list.append(i)
                ...
                if thf and low_pass[i] < -M:
                    ...
                    counter += 1
                    print(
                        "&& counter+1 ##, i:{}, -M:{}, low_pass[i]:{}, counter:{} ".format(
                            i, -M, low_pass[i], counter
                        )
                    )

                elif low_pass[i] > -M and thf:
                    print(
                        "&& climb from bottom, i:{}, -M:{}, low_pass[i]:{} ".format(
                            i, -M, low_pass[i]
                        )
                    )
                    counter = 0
            ...
            if counter > neg_threshold:
                ...
                max_dvdts.append(
                    self.delay
                    + np.argmax(ori_diff_section)
                    + thi_list[-1]
                    - int(0.01 * self.fs)
                )
                min_dvdts.append(
                    self.delay
                    + np.argmin(ori_diff_section)
                    + thi_list[-1]
                    - int(0.01 * self.fs)
                )
                counter = 0
        ...
        print("type: ", type(unfiltered_ecg))
    ...
    def ecg_detector_basic(self, unfiltered_ecg):
        f1 = 48 / self.fs
        f2 = 52 / self.fs
        b, a = signal.butter(4, [f1 * 2, f2 * 2], btype="bandstop")
        ...
        for i in range(len(filtered_ecg)):
            ...
            # No detection is allowed 200 ms after the current one
                ...
                # a new value of M5 is calculated:
                newM5 = 0.6 * np.max(filtered_ecg[QRS[-1] : i])
                # The estimated newM5 value can become quite high, if steep slope premature ventricular contraction
                # or artifact appeared, and for that reason it is limited to
                if newM5 > 1.5 * MM[-1]:
                    newM5 = 1.1 * MM[-1]
            ...
            # potential peak shows up
            if not QRS and filtered_ecg[i] > M:
                print("########################### ", i)
                QRS.append(i)
                thi_list.append(i)
                thi = True

            elif QRS and i > QRS[-1] + ms200 and filtered_ecg[i] > M:
                print("************************** ", i)
                QRS.append(i)
                thi_list.append(i)
                thi = True
            ...
            if thi and i < thi_list[-1] + ms160:
                print("111, i: ", i)
                if filtered_ecg[i] < -M and filtered_ecg[i - 1] > -M:
                    print(
                        "## close to bottom, i:{}, -M:{}, low_pass[i]:{} ".format(
                            i, -M, filtered_ecg[i]
                        )
                ...
                if thf and filtered_ecg[i] < -M:
                    thf_list.append(i)
                    counter += 1
                    print(
                        "&& counter+1 ##, i:{}, -M:{}, low_pass[i]:{}, counter:{} ".format(
                            i, -M, filtered_ecg[i], counter
                        )
                    )

                elif filtered_ecg[i] > -M and thf:
                    print(
                        "&& climb from bottom, i:{}, -M:{}, low_pass[i]:{} ".format(
                            i, -M, filtered_ecg[i]
                        )
